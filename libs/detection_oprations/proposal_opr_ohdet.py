...
def postprocess_detctions(rpn_bbox_pred, rpn_cls_prob, rpn_head_prob, rpn_angle_prob, anchors, is_training):
    ...
    for j in range(0, cfgs.CLASS_NUM):
        ...
        rpn_head_prob_ = tf.gather(rpn_head_prob, indices)
        rpn_angle_prob_ = tf.gather(rpn_angle_prob, indices)

        head_cls = tf.cast(tf.argmax(rpn_head_prob_, axis=1), tf.float32)
        ...
        tmp_boxes_pred_head = tf.reshape(tf.gather(head_cls, nms_indices), [-1, 1])
