...
def worker(gpu_id, images, det_net, args, result_queue):
    ...
    with tf.Session(config=config) as sess:
        ...
        for img_path in images:
            if 'P1083' not in img_path:
                continue
            # if img_path.split('/')[-1].split('.')[0] not in ['P0474', 'P0756', 'P1083', 'P1873', 'P2037', 'P2229', 'P2329', 'P2636']:
            #     continue
...
def eval(num_imgs, args):
    ...
    csl = build_whole_network_ohdet.DetectionNetwork(base_network_name=cfgs.NET_NAME,
                                                     is_training=False)
